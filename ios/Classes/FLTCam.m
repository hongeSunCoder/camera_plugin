...
@interface FLTCam () <AVCaptureVideoDataOutputSampleBufferDelegate,
                      AVCaptureAudioDataOutputSampleBufferDelegate, AVCaptureMetadataOutputObjectsDelegate>
...
@property(readonly, nonatomic)AVCaptureMetadataOutput* captureMetaOutput;
...
- (void)startScan {
    ...
    _captureMetaOutput = [AVCaptureMetadataOutput new];
    if ([_captureSession canAddOutput:_captureMetaOutput]) {
        [_captureSession addOutput:_captureMetaOutput];
        [_captureMetaOutput setMetadataObjectsDelegate:self queue:_captureSessionQueue];
        _captureMetaOutput.metadataObjectTypes = @[AVMetadataObjectTypeQRCode, AVMetadataObjectTypeEAN13Code, AVMetadataObjectTypeEAN8Code, AVMetadataObjectTypeUPCECode, AVMetadataObjectTypeAztecCode];
        
        CGRect interestRect = CGRectMake(0.25, 0.25, 0.5, 0.5);
        [_captureMetaOutput setRectOfInterest: interestRect];
...
- (void)stopScan {
    [_captureMetaOutput setRectOfInterest:CGRectZero];
}

- (void)captureOutput:(AVCaptureOutput *)output didOutputMetadataObjects:(NSArray<__kindof AVMetadataObject *> *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    
    AVMetadataObject *resultObject = [metadataObjects firstObject];
    NSString *resultString = ((AVMetadataMachineReadableCodeObject*)resultObject).stringValue;
    NSLog(@"scanResultStr: %@", resultString);
    
    [_mainMethodChannel invokeMethod:@"onScanCaptured" arguments:resultString];
